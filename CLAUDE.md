...
**Mel config must match ECAPA training config.** `dragon_tts/speaker_tokenizer/data/mel.py` (`LogMelSpectrogram`) reproduces SparkVox's BiCodec settings: 24 kHz / n_fft=1024 / win=640 / hop=320 / 128 mels / slaney norm + slaney scale / `log(mel.clamp(min=1e-5))`. If ECAPA was trained with different mel parameters, x_vectors will silently degrade. Verify against the source config when loading a new checkpoint.
